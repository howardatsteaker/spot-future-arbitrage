...
while std_mult < Decimal("3.0"):
    ...
    concat_close["fast_ema"] = (
        concat_close["basis"].ewm(span=config["macd_fast_length"]).mean()
    )
    concat_close["slow_ema"] = (
        concat_close["basis"].ewm(span=config["macd_slow_length"]).mean()
    )
    concat_close["dif"] = concat_close["fast_ema"] - concat_close["slow_ema"]
    concat_close["macd"] = (
        concat_close["dif"].ewm(span=config["macd_signal_length"]).mean()
    )
    concat_close["dif_sub_macd"] = concat_close["dif"] - concat_close["macd"]
    concat_close["std"] = (
        concat_close["dif_sub_macd"].rolling(config["macd_std_length"]).std()
    )

    def get_current_dif_sub_macd(
        basis: Decimal, fast_ema: Decimal, slow_ema: Decimal, macd: Decimal
    ) -> Decimal:
    ...
    while True:
        ...
        if state.spot_position > 0:
            ...
            is_macd_close = (
                dif_sub_macd < -config["macd_std_mult"] * std and entry_basis > basis
            )
