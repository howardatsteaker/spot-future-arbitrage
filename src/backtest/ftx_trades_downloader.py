...
def get_one_day_trades_from_ftx(symbol: str, start: float) -> List[Trade]:
    ...
    while True:
        ...
        min_time: str = min(data, key=lambda x: dateutil.parser.parse(x["time"]))[
            "time"
        ]
...
def run_trades_data_download_process(
    market: str, start_ts: int, end_ts: int, save_path: str
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--start_ts", required=True, type=int, help="start timestamp"
    )
    parser.add_argument("-e", "--end_ts", required=True, type=int, help="end timestamp")
    parser.add_argument(
        "-d", "--dir", required=True, help="directory to save trades data"
    )
