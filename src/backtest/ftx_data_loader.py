...
class DataLoader():
    ...
    def get_trades_generator(self, start: int, end: int) -> Generator[Trade, None, None]:
        files = filter(lambda x: start <= self.parse_filename_start_timestamp(x) < end, self.files)
        for file in files:
    ...
    def trades_df_2_klines(df: pd.DataFrame, resolution: str) -> pd.DataFrame:
        ...
        resampled_df: pd.DataFrame = df.resample(rule=resolution).agg({'price': 'ohlc', 'size': 'sum'})
