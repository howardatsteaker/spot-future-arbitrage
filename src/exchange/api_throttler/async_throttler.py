...
from src.exchange.api_throttler.async_request_context_base import (
    MAX_CAPACITY_REACHED_WARNING_INTERVAL, AsyncRequestContextBase)
from src.exchange.api_throttler.async_throttler_base import AsyncThrottlerBase
            ...
            for rate_limit, weight in self._related_limits:
                capacity_used: int = sum(
                    [
                        ...
                        and now
                        - task.timestamp
                        - (task.rate_limit.time_interval * self._safety_margin_pct)
                ...
                if capacity_used + weight > rate_limit.limit:
                    if (
                        ...
                        < now - MAX_CAPACITY_REACHED_WARNING_INTERVAL
                    ):
                        msg = (
